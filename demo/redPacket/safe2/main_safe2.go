 ...
 * 优化 channel 的吞吐量，启动多个处理协程来执行 channel 的消费 需要把packageList也拆分成16份，不然有并发安全问题
 */
package main
...
// 初始化日志
func initLog() {
	f, _ := os.Create("demo/log/lottery_demo.log")
...
func (c *lotteryController) GetGet() string {
	...
	if money <= 0 {
		...
		logger.Printf("redPacket success uid=%d, id=%d, money=%d\n", uid, id, money)
